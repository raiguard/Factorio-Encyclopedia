...
function self.listbox_with_label(parent, frame_height, label_caption)
  ...
  local label = flow.add{type='label', style='fe_listbox_label', caption=label_caption}
  local listbox_frame = flow.add{type='frame', style='fe_listbox_frame'}
  if frame_height then
    listbox_frame.style.height = frame_height
  end
  local listbox = listbox_frame.add{type='list-box', style='fe_listbox'}
...
function self.info_table_entry(parent, name, value)
  ...
  local flow = parent.add{type='flow', style='fe_vertically_centered_flow', direction='horizontal'}
  local label = flow.add{type='label', style='fe_table_label', caption={'fe-gui.'..name:gsub('_', '-')}}
  flow.add{type='empty-widget', style='fe_horizontal_pusher'}
  local value_label
  if value then
     value_label = flow.add{type='label', style='fe_table_value', caption=value}
  end
